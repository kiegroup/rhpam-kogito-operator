...
void setupDeployJob(String jobFolder, KogitoJobType jobType) {
    def jobParams = getJobParams('rhpam-kogito-operator', jobFolder, 'Jenkinsfile.deploy', 'RHPAM Kogito Operator Deploy')
...
void setupPromoteJob(String jobFolder, KogitoJobType jobType) {
    KogitoJobTemplate.createPipelineJob(this, getJobParams('rhpam-kogito-operator-promote', jobFolder, 'Jenkinsfile.promote', 'Kogito Cloud Operator Promote')).with {
        parameters {
            stringParam('DISPLAY_NAME', '', 'Setup a specific build display name')
