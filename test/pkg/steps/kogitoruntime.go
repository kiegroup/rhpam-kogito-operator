...
func (data *Data) deployExampleServiceFromRuntimeRegistry(runtimeType, kogitoApplicationName string) error {
	imageTag := data.ScenarioContext[steps.GetBuiltRuntimeImageTagContextKey(kogitoApplicationName)]
	...
	return framework.DeployRuntimeService(data.Namespace, kogitoRuntime)
}
...
func (data *Data) kogitoRuntimeHasPodsRunningWithinMinutes(dName string, podNb, timeoutInMin int) error {
	if err := kogitoFramework.WaitForDeploymentRunning(data.Namespace, dName, podNb, timeoutInMin); err != nil {
		return err
	}
	...
	return kogitoFramework.WaitForPodsWithLabel(data.Namespace, kogitoCoreFramework.LabelAppKey, dName, podNb, timeoutInMin)
}
...
func (data *Data) scaleKogitoRuntimeToPodsWithinMinutes(name string, nbPods, timeoutInMin int) error {
	...
	return kogitoFramework.WaitForDeploymentRunning(data.Namespace, name, nbPods, timeoutInMin)
}

// Logging steps
func (data *Data) kogitoRuntimeLogContainsTextWithinMinutes(dName, logText string, timeoutInMin int) error {
	return kogitoFramework.WaitForAllPodsByDeploymentToContainTextInLog(data.Namespace, dName, logText, timeoutInMin)
}
