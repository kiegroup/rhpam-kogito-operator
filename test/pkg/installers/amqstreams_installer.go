...
var (
	...
	amqStreamsOlmClusterWideInstaller = installers.OlmClusterWideServiceInstaller{
...
func GetAmqStreamsInstaller() installers.ServiceInstaller {
	return &amqStreamsOlmClusterWideInstaller
}

func getAmqStreamsCrsInNamespace(namespace string) ([]kubernetes.ResourceObject, error) {
	crs := []kubernetes.ResourceObject{}

	amqStreams := &v1beta2.KafkaList{}
	if err := kogitoFramework.GetObjectsInNamespace(namespace, amqStreams); err != nil {
		return nil, err
	}
	...
	if err := kogitoFramework.GetObjectsInNamespace(namespace, amqStreamsTopics); err != nil {
		return nil, err
	}
